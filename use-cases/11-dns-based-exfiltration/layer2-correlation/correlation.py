...
UC-11: DNS-Based Exfiltration

Correlates DNS burst, entropy, and tunneling signals into a high-confidence
...
class DNSExfilCorrelationEngine:
    """Multi-signal DNS exfiltration correlation."""

    def __init__(self, rules_file="rules.json", signals_file="../signals-output.json"):
        self.rules_file = Path(rules_file)
        self.signals_file = Path(signals_file)
        self.db_path = Path("../../../output/dns-exfil-detections.db")
    ...
    def _init_db(self):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS dns_exfil_detections (
    ...
    def correlate(self, signals: List[Dict], rules: Dict):
        """Correlate DNS exfiltration signals by source IP."""
        print("\n" + "=" * 60)
        print("LAYER 2: DNS Exfiltration Correlation")
        print("=" * 60)
        ...
        for src_ip, ip_signals in signals_by_ip.items():
            ...
            if required_signals.issubset(signal_types):
                ...
                if self._validate_time_window(ip_signals, time_window_minutes):
                    ...
                    print(f"  Tunnel Domain: {detection['tunnel_domain']}")
                    print(f"  Severity: {detection['severity']}")
                    print(f"  Signals: {detection['signal_count']}")
                    print(f"  Encoded Chunks: {detection['encoded_chunk_count']}")
                else:
    ...
    def _create_detection(self, src_ip: str, signals: List[Dict], rules: Dict) -> Dict:
        """Create a DNS exfiltration detection record."""
        ...
        for signal in signals:
            ...
            if signal_type == "ENCODED_TUNNELING_PATTERN":
                tunnel_domain = signal.get("tunnel_domain", tunnel_domain)
                encoded_chunk_count = signal.get("encoded_chunk_count", 0)
            elif signal_type == "HIGH_ENTROPY_DNS":
                suspicious_query_count = signal.get("suspicious_query_count", 0)
            elif signal_type == "DNS_QUERY_BURST":
                query_count = signal.get("query_count", 0)
        ...
        return {
            ...
            "suspicious_query_count": suspicious_query_count,
    ...
    def save_detections(self):
        ...
        try:
            for detection in self.detections:
                cursor.execute("""
                    INSERT OR REPLACE INTO dns_exfil_detections
                    (detection_id, detection_type, src_ip, tunnel_domain, severity,
                     confidence, signal_count, encoded_chunk_count,
                     suspicious_query_count, query_count, timestamp, signals_json)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    detection["detection_id"],
                    detection["detection_type"],
                    detection["src_ip"],
                    detection["tunnel_domain"],
                    detection["severity"],
                    detection["confidence"],
                    detection["signal_count"],
                    detection["encoded_chunk_count"],
                    detection["suspicious_query_count"],
            ...
            count = cursor.execute(
                "SELECT COUNT(*) FROM dns_exfil_detections"
            ).fetchone()[0]
            print(f"[VERIFY] {count} DNS exfiltration detection(s) in database")
    ...
    def _print_summary(self):
        ...
        print("\n" + "=" * 60)
        print("DNS Exfiltration Detection Summary")
        print("=" * 60)
        ...
        for detection in self.detections:
            ...
            print(f"Signals: {detection['signal_count']}")
            print(f"DNS Queries: {detection['query_count']}")
            print(f"Suspicious Queries: {detection['suspicious_query_count']}")
            print(f"Encoded Chunks: {detection['encoded_chunk_count']}")
...
def main():
    engine = DNSExfilCorrelationEngine()
    engine.run()
